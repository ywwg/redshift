...
static int gnomerr_init(gnomerr_state_t **state) {
  *state = malloc(sizeof(gnomerr_state_t));
  if (*state == NULL)
    return -1;

  gnomerr_state_t *s = *state;
  /* Initialize state. */
  s->crtc_num = NULL;
...
static int gnomerr_start(gnomerr_state_t *state) {
  GError *error = NULL;
  guint i, count;
...
Request current gamma ramps */
    ...
    if (!gnome_rr_crtc_get_gamma(crtcs[i], &ramp_size, &gamma_r, &gamma_g,
                                 &gamma_b)) {
...
static void gnomerr_restore(gnomerr_state_t *state) {
  /* Restore CRTC gamma ramps */
  for (int i = 0; i < state->crtc_count; i++) {
    guint32 crtc_id = state->crtcs[i].crtc;
    GnomeRRCrtc *crtc = gnome_rr_screen_get_crtc_by_id(state->screen, crtc_id);
    ...
    /* Set gamma ramps */
    if (!gnome_rr_crtc_set_gamma(crtc, ramp_size, gamma_r, gamma_g, gamma_b)) {
      g_printerr(_("`%s' failed for crtc id %d\n"), "GNOMERR Set CRTC Gamma",
                 crtc_id);
...
static void gnomerr_free(gnomerr_state_t *state) {
  /* Free CRTC state */
  for (int i = 0; i < state->crtc_count; i++) {
...
static void gnomerr_print_help(FILE *f) {
...
static int gnomerr_set_option(gnomerr_state_t *state, const char *key,
                              const char *value) {
  if (strcasecmp(key, "crtc") == 0) {
    ...
    g_printerr(_("preserve flag no longer supported\n"));
  } else {
...
static int gnomerr_set_temperature_for_crtc(gnomerr_state_t *state,
                                            int crtc_num,
  ...
  if (crtc_num >= state->crtc_count || crtc_num < 0) {
    g_printerr(_("CRTC %d does not exist. "), crtc_num);
...
static int gnomerr_set_temperature(gnomerr_state_t *state,
                                   const color_setting_t *setting,
                                   int preserve) {
  int r;
...
set temperature on all CRTCs. */
  if (state->crtc_num_count == 0) {
    for (int i = 0; i < state->crtc_count; i++) {
      r = gnomerr_set_temperature_for_crtc(state, i, setting, preserve);
      if (r < 0)
        return -1;
    }
  } else {
    for (int i = 0; i < state->crtc_num_count; ++i) {
      r = gnomerr_set_temperature_for_crtc(state, state->crtc_num[i], setting,
                                           preserve);
      if (r < 0)
        return -1;
    }
...
const gamma_method_t gnomerr_gamma_method = {
    "gnomerr",
    1,
