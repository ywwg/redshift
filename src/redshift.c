...
#undef CLAMP
#define CLAMP(lo, mid, up)                                                     \
  (((lo) > (mid)) ? (lo) : (((mid) < (up)) ? (mid) : (up)))
...
static period_t get_period_from_time(const transition_scheme_t *transition,
                                     int time_offset) {
...
static period_t get_period_from_elevation(const transition_scheme_t *transition,
                                          double elevation) {
...
get_transition_progress_from_time(const transition_scheme_t *transition,
  ...
  if (time_offset < transition->dawn.start ||
    ...
    return (transition->dawn.start - time_offset) /
           (double)(transition->dawn.start - transition->dawn.end);
  } else if (time_offset > transition->dusk.start) {
    return (transition->dusk.end - time_offset) /
           (double)(transition->dusk.end - transition->dusk.start);
  } else {
    return 1.0;
  }
}
...
static int get_seconds_since_midnight(double timestamp) {
...
static void print_period(period_t period, double transition) {
  switch (period) {
  case PERIOD_NONE:
  case PERIOD_NIGHT:
  case PERIOD_DAYTIME:
...
/* Print location */
static void print_location(const location_t *location) {
  ...
  printf(_("Location: %.2f %s, %.2f %s\n"), fabs(location->lat),
         location->lat >= 0.f ? north : south, fabs(location->lon),
         location->lon >= 0.f ? east : west);
}

/* Interpolate color setting structs given alpha. */
static void interpolate_color_settings(const color_setting_t *first,
                                       const color_setting_t *second,
                                       double alpha, color_setting_t *result) {
  alpha = CLAMP(0.0, alpha, 1.0);

  result->temperature =
      (1.0 - alpha) * first->temperature + alpha * second->temperature;
  result->brightness =
      (1.0 - alpha) * first->brightness + alpha * second->brightness;
  for (int i = 0; i < 3; i++) {
    result->gamma[i] =
        (1.0 - alpha) * first->gamma[i] + alpha * second->gamma[i];
  }
}

/* Interpolate color setting structs transition scheme. */
static void interpolate_transition_scheme(const transition_scheme_t *transition,
                                          double alpha,
...
static int color_setting_diff_is_major(const color_setting_t *first,
                                       const color_setting_t *second) {
...
static void color_setting_reset(color_setting_t *color) {
...
static int provider_try_start(const location_provider_t *provider,
...
static int method_try_start(const gamma_method_t *method, gamma_state_t **state,
                            config_ini_state_t *config, char *args) {
  int r;
  ...
  config_ini_section_t *section = config_ini_get_section(config, method->name);
  if (section != NULL) {
    config_ini_setting_t *setting = section->settings;
    while (setting != NULL) {
      r = method->set_option(*state, setting->name, setting->value);
      if (r < 0) {
        method->free(*state);
        fprintf(stderr,
                _("Failed to set %s"
...
static int gamma_is_valid(const float gamma[3]) {
...
/* Check whether location is valid.
   ...
   returns 1. */
static int location_is_valid(const location_t *location) {
  /* Latitude */
...
static int provider_get_location(const location_provider_t *provider,
                                 location_state_t *state, int timeout,
                                 location_t *loc) {
  int available = 0;
...
static double ease_fade(double t) {
  if (t <= 0)
    return 0;
  if (t >= 1)
    return 1;
  return 1.0042954579734844 *
...
/* Run continual mode loop
   ...
   color temperature. */
static int run_continual_mode(const location_provider_t *provider,
                              location_state_t *location_state,
                              const transition_scheme_t *scheme,
                              const gamma_method_t *method,
                              gamma_state_t *method_state, int use_fade,
  ...
  while (1) {
    ...
    if (use_fade) {
      if ((fade_length == 0 &&
          ...
          (fade_length != 0 &&
           color_setting_diff_is_major(&target_interp, &prev_target_interp))) {
    ...
    if (fade_length != 0) {
      ...
      interpolate_color_settings(&fade_start_interp, &target_interp, alpha,
                                 &interp);
    ...
    if (loc_fd >= 0) {
      ...
      location_t new_loc;
      int new_available;
      r = provider->handle(location_state, &new_loc, &new_available);
      if (r < 0) {
        fputs(_("Unable to get location"
                " from provider.\n"),
              stderr);
      ...
      if (new_available && (new_loc.lat != loc.lat || new_loc.lon != loc.lon ||
...
int main(int argc, char *argv[]) {
  ...
  options_parse_config_file(&options, &config_state, gamma_methods,
                            location_providers);
  ...
  int need_location = options.mode != PROGRAM_MODE_RESET &&
                      options.mode != PROGRAM_MODE_MANUAL &&
                      !options.scheme.use_time;
  if (need_location) {
    if (options.provider != NULL) {
      /* Use provider specified on command line. */
      r = provider_try_start(options.provider, &location_state, &config_state,
    ...
    if (options.scheme.high < options.scheme.low) {
      fprintf(stderr, _("High transition elevation cannot be lower than"
  ...
  if (options.mode != PROGRAM_MODE_RESET &&
    ...
    if (options.scheme.day.temperature < MIN_TEMP ||
      ...
      fprintf(stderr, _("Temperature must be between %uK and %uK.\n"), MIN_TEMP,
              MAX_TEMP);
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (options.scheme.day.brightness < MIN_BRIGHTNESS ||
    ...
    fprintf(stderr, _("Brightness values must be between %.1f and %.1f.\n"),
  ...
  if (!gamma_is_valid(options.scheme.day.gamma) ||
    ...
    fprintf(stderr, _("Gamma value must be between %.1f and %.1f.\n"),
            MIN_GAMMA, MAX_GAMMA);
    exit(EXIT_FAILURE);
  }
  ...
  if (options.verbose) {
    ...
    printf(_("Gamma (%s): %.3f, %.3f, %.3f\n"), _("Daytime"),
           options.scheme.day.gamma[0], options.scheme.day.gamma[1],
           options.scheme.day.gamma[2]);
    printf(_("Gamma (%s): %.3f, %.3f, %.3f\n"), _("Night"),
           options.scheme.night.gamma[0], options.scheme.night.gamma[1],
  ...
  if (options.mode != PROGRAM_MODE_PRINT) {
    if (options.method != NULL) {
      ...
      r = method_try_start(options.method, &method_state, &config_state,
                           options.method_args);
      if (r < 0)
        exit(EXIT_FAILURE);
    } else {
  ...
  case PROGRAM_MODE_PRINT: {
    ...
    if (need_location) {
      ...
      int r = provider_get_location(options.provider, location_state, -1, &loc);
      if (r < 0) {
        fputs(_("Unable to get location"
                " from provider.\n"),
              stderr);
    ...
    color_setting_t interp;
    interpolate_transition_scheme(scheme, transition_prog, &interp);
    ...
    if (options.mode != PROGRAM_MODE_PRINT) {
      /* Adjust temperature */
      r = options.method->set_temperature(method_state, &interp,
  ...
  case PROGRAM_MODE_CONTINUAL: {
    r = run_continual_mode(options.provider, location_state, scheme,
                           options.method, method_state, options.use_fade,
